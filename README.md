...
## 🌐 Web 可视化管理控制台 (与 TUI 二选一常驻)

本项目内置零依赖独立轻量 Web 管理控制台，与 TUI 终端模式形成双轨对等支持。支持配置与规则在线维护、一键触发并发抓取并实时观测执行日志，更支持作为**后台常驻守护服务**挂机运行：

```bash
python main.py web
# 或以常驻定时监控守护模式启动（默认每30分钟轮询一次，发现新条目自动触发系统桌面通知）：
...
- **定时监控守护 (Runtime Daemon)**：在控制台页面一键启停或通过 `--daemon` 随服务自启，内置轮询倒计时、轮数统计，命中合格候选条目时自动调用 HTML5 Desktop Notification 弹出系统桌面通知。
- **规则与配置管理**：直观的多选科目标签（支持一键添加自定义标签、点击 × 移除）、身份预设模板切换、年级范围下拉选择（小学1年级至高三）、特殊年级排除开关、排除限定关键词输入框，以及百度地图 AK 与 LLM 密钥在线脱敏查看与更新保存。
- **匹配候选大厅**：实时展示匹配通过的合格候选列表，支持一键打开网页、复制链接、快速标记为“已联系”。
- **未入选分析看板**：分类呈现被硬过滤、规则过滤、大模型否决的条目及精确原因。
- **任务控制台与日志**：在线触发流水线（支持大模型并发模式、Embedding 极速模式、混合模式），长轮询级实时日志滚动展示。

> **提示**：日常挂机使用时，**TUI (`zjjjzx tui`) 与 Web (`zjjjzx web -d`) 二选一即可**。两者共享同一个底层 SQLite 数据库与规则配置；喜欢在黑底终端操作的可选 TUI，喜欢在浏览器中挂机并接收桌面弹窗通知的可选 Web 守护模式。
